...
rng default 
p=700;%dimension of sample data
n=500;
...
Sigma=0.01*Sigma;
%Sigma=%rho.^(abs(xr*ones(1,p)-ones(p,1)*xr'))/4;
% Sigma=eye(p);
% s=abs(randn(1,p))*10;
% Sigma=diag(s);
...
rankC=rank(C);
k=1:10:rank(C);%reduced dimension when applying random projection
H=inv(eye(p)+gamma*C);
...
for i=1:length(k)
    ...
    error_RPplusLDA(i)=classifierLDA(xBar0_RP,xBar1_RP,H_RP,X0_test_RP,X1_test_RP,n0_test,n1_test,n_test,c);
