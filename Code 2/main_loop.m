...
rng default 
p=500;%dimension of sample data
n=80;
...
rankC=rank(C);
k=1:5:rank(C);%reduced dimension when applying random projection
H=inv(eye(p)+gamma*C);
...
%     H_RP=inv(C_RP*0.1+eye(k(i)));
    ...
    error_RLDA(i)=classifierRLDA(xBar0,xBar1,H,X0_test,X1_test,n0_test,n1_test,n_test,c,kappa);%RLDA
...
figure
plot(k,error_RLDA,k,error_RPplusLDA,k,error_RPplusLDAEnsemble)
legend('RLDA','RP+LDA','RP+LDA ensemble')
